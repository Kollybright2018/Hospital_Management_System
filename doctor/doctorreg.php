...
if(isset($_POST['apply'])){
  ...
  if ( ($password != $con_password) ) {
    ...
    $select = mysqli_query($db, "SELECT * FROM doctor WHERE  email = '$email'");
    $count = mysqli_num_rows($select);
    if ($count > 0) {
         $error['reg'] = "User Already Exist" ;   
    }else{
      $insert =mysqli_query($db, "INSERT INTO doctor (full_name, username, email, gender, phone_no, country, password, salary, reg_date, status, profile) VALUES ('$fname', '$username', '$email', '$gender', '$phone_no', '$country', md5('$password'), '66', NOW(), 'pending', 'bright.jpg')");
      if($insert){
...
<?php if (isset($error['reg'])): ?>
  	...
  	<div class="col-md-12">
        ...
        <div class="col-md-6 card">
  			<h4 class="text-center">Doctors Registration</h4>
        <?php if (isset($error['reg'])): ?>
          <div class="alert alert-danger data-dismissible">
            <button class="close" data-dismiss="alert" >&times;</button>
            <strong> <?php print_r($error['reg']) ;?></strong>
        ...
        <?php endif ?>
         ...
         <div class="form-group">
          ...
          <p class="text-center">Already a Memeber Click <a href="doctorlogin.php">Here</a> To Log in</p>
  			</div>
